...
const Header = () => {
  ...
  return <header className="sticky top-0 z-50 w-full bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/90 border-b border-border">
      <div className="container flex h-16 items-center justify-between">
        ...
        <Link to="/" className="flex items-center gap-2">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-xs leading-tight text-slate-950">Dhundho Apna Lawyer</span>
          </div>
        </Link>
      ...
      {isMenuOpen && <div className="md:hidden border-t border-border bg-background">
          <nav className="container py-4 flex flex-col gap-3">
            {navLinks.map(link => <Link key={link.href} to={link.href} className="text-sm font-medium text-muted-foreground hover:text-foreground transition-colors py-2" onClick={() => setIsMenuOpen(false)}>
                {link.label}
              </Link>)}
